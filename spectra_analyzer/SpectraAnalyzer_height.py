...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def make_ComboBox_fields(self, cb, ii):
        ...
        if cb[1].currentText() == "":
            ...
            self.LGfit.addWidget(QLabel("Height:"),ii*2+1,3)
            self.LGfit.addWidget(amp,ii*2+1,4)
    ...
    def fitmodel_setup(self):
        ...
        for nn,cb in enumerate(self.combo_mod):
            ...
            # elif cb.currentText() == "Gaussian":
                ...
                if len(amp) >= 1:
                    ...
                    if self.LGfit.itemAtPosition(nn*2+1,10).widget().isChecked():
                        self.pars[mod_name+"height"].set(value=va, max=0,vary=True)
                    else:
                        self.pars[mod_name+"height"].set(value=va, min=0,vary=True)
                else:
                    self.pars[mod_name+"height"].set(min=0,vary=True)
                    
                self.pars[mod_name+"height"].set(max =self.max_int,vary=True)
            ...
            if mod_name is not None:
                ...
                # return
    ##TODO make sure that curve names with numbers still allow fitting    
    ...
    def add_fitting_data_to_gui(self):
        ...
        for cc, key in enumerate(ke):
            ...
            if box_name == "":
                ...
                v = list(fv.keys())
                fv[v[0]]=fv[v[-1]]## To make sure height is displayed
                mod = 6
                extra = 2
